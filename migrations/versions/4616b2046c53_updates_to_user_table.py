"""updates to user table

Revision ID: 4616b2046c53
Revises: 398d8d2a9134
Create Date: 2024-06-25 19:44:45.414106

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = '4616b2046c53'
down_revision = '398d8d2a9134'
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('user', schema=None) as batch_op:
        batch_op.add_column(sa.Column('bio', sa.String(length=500), nullable=True))
        batch_op.add_column(sa.Column('pfp', sa.String(length=500), nullable=True))
        batch_op.add_column(sa.Column('pronouns', sa.String(length=50), nullable=True))
        batch_op.add_column(sa.Column('admin', sa.Boolean(), nullable=True))

    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('user', schema=None) as batch_op:
        batch_op.drop_column('admin')
        batch_op.drop_column('pronouns')
        batch_op.drop_column('pfp')
        batch_op.drop_column('bio')

    # ### end Alembic commands ###
